...
Главная учебная идея `one-fastApi` — **показать один и тот же результат несколькими способами и дать сравнить их построчно**. Это не продуктовый сервис, а «каталог приёмов». Четыре учебных блока:
