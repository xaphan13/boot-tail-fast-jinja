...
- **Сравнение архитектур:** `example_sql` показывает, как отделять HTTP-контракт от доступа к данным; `ex_order_product` показывает, как **не надо** — слой приложения отсутствует, связность роута низкая (см. 04).
...
| CRUD-слой vs SQL в роутах | Слоистая архитектура | ✅ в `example_sql`, ❌ в `ex_order_product` |
