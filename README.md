...
| Файл | Назначение |
|---|---|
| [QWEN.md](QWEN.md) | контекст проекта + инструкции оркестратора (главная сессия: glm-5.3 / gpt-5.6-sol) |
...
харнесс, та и оркестратор) по `QWEN.md` действует как
...
Модели команды (субагенты объявлены в `~/.qwen/settings.json`, authType `openai`):

| Роль | Модель |
|---|---|
| оркестратор (главная сессия) | glm-5.3 или gpt-5.6-sol — оркестратором становится модель, с которой запущена главная сессия |
| backend-dev | nordrouter/moonshotai/kimi-k2.7-code |
| frontend-dev, adversary | nordrouter/minimax/minimax-m3 |
| qa | nordrouter/xiaomi/mimo-v2.5 |
...
1. Убедитесь, что модели субагентов объявлены в `~/.qwen/settings.json`
   (`nordrouter/moonshotai/kimi-k2.7-code`, `nordrouter/minimax/minimax-m3`,
   `nordrouter/xiaomi/mimo-v2.5`), а главная сессия запущена на `glm-5.3` или
   `gpt-5.6-sol` — с какой запущен харнесс, та и оркестратор.
