...
async def lifespan(_: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Если SECRET_KEY не задан в .env, model_validator в app/core/config.py
    # сгенерировал его на лету: JWT-сессии инвалидируются при каждом
    # рестарте. Это переносимый шаблон, но в прод так оставлять нельзя —
    # пользователи будут разлогиниваться без видимой причины.
    if settings.SECRET_KEY_GENERATED:
        logger.warning(
            "SECRET_KEY was auto-generated at runtime; JWT sessions will "
            "be invalidated on every restart. Set SECRET_KEY in your .env "
            "(e.g. `openssl rand -hex 32`) to keep sessions stable.",
        )
