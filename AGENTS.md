...
- Когда все критерии успеха подтверждены, оркестратор архивирует задание:
  ...
  по умолчанию только «Итог» (дата закрытия, 1–2 предложения, ссылка на ключевой
  артефакт); секции «Дефекты», «Adversarial-прогон», «Участники» добавляются только
  если есть существенное содержание. Шаблон отчёта — в QWEN.md. Затем создаёт
  свежую заглушку `tasks/current/REQUIREMENTS.md`
